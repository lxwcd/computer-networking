...
路由器本质上就是一台专用小型计算机
核心：路由器 = 阉割定制化的嵌入式计算机
一台标准计算机组成：**CPU、内存RAM、存储Flash、操作系统、网卡**
路由器全部都有：
1. CPU：嵌入式处理器（MIPS/ARM），性能一般，专门处理数据包转发
2. 内存RAM：运行内存，跑系统、存路由表、连接状态
3. Flash闪存：相当于硬盘，存放路由器操作系统固件
4. 网络接口：WAN口、LAN口，就是网卡（多网卡）
5. 操作系统：**嵌入式Linux居多**（小米、TP‑Link大部分基于OpenWrt/定制Linux）

> 普通通用电脑：用途通用，可以浏览器、办公、装各种软件。
> 路由器：硬件简化，操作系统裁剪，**主要任务只有一个：数据包转发（路由）**。

- PC通用计算机：什么活都能干
- 路由器：一台**专门干网络转发的专用小电脑**，出厂固件锁死，普通用户不能随便装普通桌面软件。

1. **路由器设备（硬件）**：是计算机硬件。
2. **路由功能（软件逻辑）**：操作系统里面的程序，负责查表转发IP数据包。

👉一台普通Linux服务器，配多张网卡，开启内核ip转发`net.ipv4.ip_forward=1`，**普通Linux主机也能当路由器用**。
> 也就是说普通服务器也具备路由器软件能力，只是硬件没有专门优化。

家用TP‑Link、小米路由器：
- 底层大多是Linux内核，但厂商做大量裁剪；
- 不给你完整shell，web页面管理；**普通用户不能apt/yum随便装软件**。
> 但刷OpenWrt固件之后，就变成完整嵌入式Linux，可以ssh登录，安装各种软件包。

路由器和交换机本质差别：
- **路由器（三层设备，网络层）**：IP路由，跨网段转发，识别IP地址；有CPU、内存，是小型计算机。WAN/LAN。
- **交换机（二层设备，数据链路层）**：主要硬件芯片转发，几乎没有通用CPU；只看MAC地址，同网段内部转发。简单傻瓜交换机算不上计算机。

> ⚠️注意：高端企业级交换机也带管理CPU，但数据转发大多靠专用硬件芯片。家用傻瓜交换机几乎没有可编程处理器，不算计算机。

光猫同样也是嵌入式小型计算机，带光信号转换芯片+CPU内存Linux系统。
