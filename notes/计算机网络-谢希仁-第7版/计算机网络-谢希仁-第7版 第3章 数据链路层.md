...
- MAC 地址在适配器的 ROM 中，软件地址（IP 地址）在计算机的存储器中。
...
关于环路的形成和生成树协议查看 **趣谈网络协议 2.2 节** 的讲解
...
- 以太网规定一个帧最短长度，64 字节，不足则填充一些字节；总共 64 字节，减去 18 字节首部和尾部字段，则数据字段最短为 46 字节。最初定义这个是因为执行 CSMA/CD 时，帧太短可能造成无法检测到发生碰撞。
...
- 前同步码用于让适配器接收 MAC 帧时能迅速调整其时钟频率，使其和发送端的时钟同步
